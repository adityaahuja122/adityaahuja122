## Hi, I'm Aditya 👋

A backend developer who enjoys building reliable systems and working closely with teams to deliver clean, well-structured solutions. I value collaboration, clear communication, and writing code that is easy to maintain and grow.

### ⚙️ Tech Stack
- PHP, Laravel, MySQL, Redis
- JavaScript, React
- REST APIs, JWT
- WordPress (learning – plugins, hooks & filters)

### 🌱 Currently Learning
- WordPress core & plugin architecture
- Reading large open-source PHP codebases

### ✍️ Technical Writing & Knowledge Sharing
- Co-authored a technical blog during an internship explaining the setup and integration of **Laravel Reverb with a React frontend**.
- Covered backend configuration, frontend setup, and real-time event flow with practical implementation details.

🔗 Blog: https://medium.com/@raheja.saachi04114/how-to-integrate-laravel-reverb-with-react-from-setup-to-real-time-success-293db64a3822

### 🔗 Links
- GitLab (main work): https://gitlab.com/adityaahuja020
- Resume: https://drive.google.com/file/d/19_i1kJSkWgzdw2khqOfWA7fylNwUOolF/view?usp=sharing
- LinkedIn: https://www.linkedin.com/in/aditya-ahujaa/
